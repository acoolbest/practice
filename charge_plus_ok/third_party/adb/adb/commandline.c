...
//add by zhzq @ 2016-3-9
static void read_and_dump_for_install(int fd)
{
    char buf[4096];
    int len;
	fd_set rset;
    ...
    while(fd >= 0) {
		...
		if(ret == -1)
		{
			fprintf(stderr,"Failure [INSTALL_FAILED_INTERNAL_ERROR]\n");
			break;
		}
		else if(ret == 0)
		{
			fprintf(stderr,"Failure [INSTALL_FAILED_VERIFICATION_TIMEOUT]\n");
			break;
		}
		else
		{
			if(FD_ISSET(fd,&rset))
			{
				//printf("read_and_dump(): pre adb_read(fd=%d)\n", fd);
		        D("read_and_dump(): pre adb_read(fd=%d)\n", fd);
		        len = adb_read(fd, buf, 4096);
				//printf("read_and_dump(): post adb_read(fd=%d): len=%d\n", fd, len);
		        D("read_and_dump(): post adb_read(fd=%d): len=%d\n", fd, len);
		        if(len == 0) {
		            break;
		        }
...
//add by zhzq @ 2016-3-9
static int send_shellcommand_for_nowait(transport_type transport, char* serial, char* buf)
{
	int fd, ret = -1;
...
//add by zhzq @ 2016-3-9
static int send_shellcommand_for_install(transport_type transport, char* serial, char* buf)
{
    int fd, ret = -1;
...
static int logcat(transport_type transport, char* serial, int argc, char **argv)
{
    ...
    send_shellcommand_for_nowait(transport, serial, buf);//change by zhzq @ 2016-3-10
    return 0;
}
...
//add by zhzq @ 2016-3-9
static int pm_command_for_install(transport_type transport, char* serial,
                      int argc, char** argv)
{
    char buf[4096];
    ...
    send_shellcommand_for_install(transport, serial, buf);
    return 0;
}
...
int install_app(transport_type transport, char* serial, int argc, char** argv)
{
	...
	//change by zhzq @ 2016-3-9
    pm_command_for_install(transport, serial, argc, argv);
