...
void show_help_children(const AVClass *showclass, int flags)
{
    const AVClass *child = NULL;
    if (showclass->option) {
        av_opt_show2(&showclass, NULL, flags, 0);
        printf("\n");
    }

    while ((child = av_opt_child_class_next(showclass, child)))
...
static const OptionDef *find_option(const OptionDef *po, const char *name)
{
    ...
    unsigned int len = p ? p - name : strlen(name);
                        ...
                        const char *arg)
{
    ...
    if (po->flags & OPT_SPEC) {
        ...
        dstcount = (int *)(so + 1);
        *so = (SpecifierOpt *)grow_array((void *)(*so), sizeof(**so), dstcount, *dstcount + 1);
        str = av_strdup(p ? p + 1 : "");
        if (!str)
        ...
        *(double *)dst = parse_number_or_die(opt, arg, OPT_DOUBLE, -INFINITY, INFINITY);
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR,
                   ...
                   arg, opt, av_make_error_string(errbuf, 64, ret));
            return ret;
        }
    }
...
static void expand_filename_template(AVBPrint *bp, const char *template_zhzq,
                                     struct tm *tm)
{
    int c;

    while ((c = *(template_zhzq++))) {
        if (c == '%') {
            if (!(c = *(template_zhzq++)))
                break;
            switch (c) {
