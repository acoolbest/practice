...
void *combine_atoms(char *name, int amount, ...)
{
    ...
    {
        void *atom = va_arg(vl, void *);
        uint32_t content = htonl(*(uint32_t *)atom);
...
void *generate_supp(uint32_t *suppsize)
{
    ...
    uint32_t *supp = (uint32_t *)malloc(100612);
    *suppsize = 100612;
...
void *generate_sinf(int appid, char *person_name, int vendorID, uint32_t & out_length)
{
    ...
    uint32_t *fakepriv = (uint32_t *)malloc(440);
    ...
    cout << buf_len << endl;
    // sinf.frma = "game"
    void *frma = create_atom("frma", 4, (void *)"game");
    cout << buf_len << endl;
    // sinf.schm = "\x00000000itun\x00000000"
    void *schm = create_atom("schm", 12, (void *)"\x00\x00\x00\x00itun\x00\x00\x00\x00");
    cout << buf_len << endl;
    uint32_t *sign = (uint32_t *)malloc(128);
