...
static int recv_buffer_from_fd(int socket_fd, unsigned char *recv_buffer, int *recv_length)
{
	int rev = -1;
...
void* respons_stb_info(void *parameter, int thread_index)
{
	char log_str_buf[LOG_STR_BUF_LEN];
	char send_buffer[1024] = "i have get you data";
	int sockfd = ((thpool_job_funcion_parameter *)parameter)->fd;
	printf("get buffer = %s\n", ((thpool_job_funcion_parameter *)parameter)->recv_buffer);
	printf("sockfd = %d\n", sockfd);
