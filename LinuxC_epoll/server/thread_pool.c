...
static void *thpool_thread_do(void *arg);
...
static void *thpool_thread_do(void *arg)
{
	thpool_thread_parameter thread_paramter = *((thpool_thread_parameter *)arg);
	free((thpool_thread_parameter *)arg);
	arg = 0;
...
// add to thread pool
int thpool_add_work(thpool_t* tp_p, void *(*function_p)(void* arg, int index), /*void *arg_p*/int socket_fd, unsigned char *recev_buffer)
{
