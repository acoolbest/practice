...
typedef struct {
	uint32_t msg_id;
	uint32_t from_id;
	uint32_t to_id; //generally for group
	uint64_t timestamp;
...
public:
	...
	void AddToSendList(uint32_t msg_id, uint32_t from_id, uint32_t to_id, uint32_t sequence_no, uint32_t session_type);
	void DelFromSendList(uint32_t msg_id, uint32_t from_id, uint32_t to_id, uint32_t sequence_no, uint32_t session_type);
	int GetSendList(void);
	void ResendMessageFromSendList(void);
	void ResendMessageFromSendList(list<msg_ack_t>::iterator &it);
	void SetSentStatus(bool bSentStatus);
	bool GetSentStatus(void);
	bool AddToRecvList(uint32_t msg_id, uint32_t from_id, uint32_t to_id, uint32_t sequence_no, uint32_t session_type);
	void DelFromRecvList(uint64_t ullCurrTime);
	bool ResendMessageFromRecvList(uint32_t sequence_no);
	void ResendMessageFromRecvList(list<msg_ack_t>::iterator &it);
private:
	...
	void _HandleClientUpdateUserNagure(CImPdu* pPdu);
	void _HandleClientUpdateUser(CImPdu* pPdu);
	...
	void _HandleClientCompanyRequest(CImPdu* pPdu);
    void _SendFriendStatusNotify(uint32_t status);
	void _HandleOnlineUsersRequest(CImPdu* pPdu);
	void _HandleClientShieldUserRequest(CImPdu* pPdu);
	bool IsValidClientVersion(void);
	bool IsAllowdedClientVersion(const char* pszCurrVersion);

	void _HandleClientPositionRequest(CImPdu* pPdu);
	void _HandleClientAllUserLiteRequest(CImPdu* pPdu);
	void _HandleClientUserInfoLiteRequest(CImPdu* pPdu);

	//added by george on July 7th,2016
	void _HandleClientAllStaffRequest(CImPdu* pPdu);
	void _HandleClientAllStaffLiteRequest(CImPdu* pPdu);
	void _HandleClientStaffInfoRequest(CImPdu* pPdu);
	void _HandleClientUpdateStaffRequest(CImPdu* pPdu);
	void _HandleClientChangeVipRequest(CImPdu* pPdu);
	void _HandleClientAssistantInfoRequest(CImPdu* pPdu);
	void _HandleClientQueryUserInfoRequest(CImPdu* pPdu);
...
private:
	...
	list<msg_ack_t> m_recv_msg_list;
	...
	bool m_bSent; //can send a message
};
