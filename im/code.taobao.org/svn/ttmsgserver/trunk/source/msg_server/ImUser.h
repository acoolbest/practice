...
public:
	...
	void BroadcastPduByLoginTime(CImPdu* pPdu);
	...
	void BroadcastClientMsgData(CImPdu* pPdu, uint32_t msg_id, uint32_t to_id, uint32_t sequence_no, CMsgConn* pFromConn = NULL, uint32_t from_id = 0);
    void BroadcastData(void* buff, uint32_t len, CMsgConn* pFromConn = NULL);
	void BroadcastData(void* buff, uint32_t len, uint32_t msg_id, uint32_t to_id, uint32_t sequence_no, CMsgConn* pFromConn = NULL, uint32_t from_id = 0);
...
public:
	...
	void BroadcastPdu(CImPdu* pdu, uint32_t client_type_flag, CMsgConn *pMsgConn = NULL);
private:
