...
#define IOS_PUSH_FLASH_MAX_LENGTH    128
...
void build_ios_push_flash(string& flash, uint32_t msg_type, uint32_t from_id)
{
    ...
    else
    {
		...
		if (pImUser)
		{
			...
			else if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_FILE)
			{
				msg_tmp.append(nick_name);
				msg_tmp.append("在群聊中发送了一个文件");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_FILE)
			{
				msg_tmp.append(nick_name);
				msg_tmp.append("给您发送了一个文件");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_FILE_STATUS)
			{
				msg_tmp.append(nick_name);
				msg_tmp.append("已成功收发一个文件");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_FILE_STATUS)
			{
				;
			}
			else
        ...
        {
			if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_SYSTEM)
			{
				msg_tmp.append("您收到一条个人系统消息");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_SYSTEM)
			{
				msg_tmp.append("您收到一条群系统消息");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_WORKFLOW)
			{
				msg_tmp.append("您收到一条审批消息");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_WORKFLOW)
			{
				;
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_EMAIL)
			{
				msg_tmp.append("您收到一封邮件");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_EMAIL)
			{
				;
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_BULLETIN)
			{
				msg_tmp.append("您收到一条公告消息");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_BULLETIN)
			{
				;
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_NEW_FRIEND)
			{
				msg_tmp.append("您有一个新的好友");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_NEW_STAFF)
			{
				msg_tmp.append("您有新的团队成员加入申请");
			}
			else if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_ATTENDANCE)
			{
				msg_tmp.append("您有一个考勤提醒");
			}
			else
			{
				msg_tmp = comm_flash;
