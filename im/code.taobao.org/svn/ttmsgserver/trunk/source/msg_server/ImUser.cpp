...
void CImUser::BroadcastPdu(CImPdu* pPdu, CMsgConn* pFromConn)
{
	...
	{
		CMsgConn* pConn = it->second;
		if (pConn && pConn != pFromConn && pConn->GetUserId() != 0)
		{
			pConn->SendPdu(pPdu);
			log("3.============================================User: %d, sending a packet to %d=============================", m_user_id, pConn->GetUserId());
		}
	}
}
...
void CImUser::BroadcastPduWithOutMobile(CImPdu *pPdu, CMsgConn* pFromConn)
{
    ...
    {
        CMsgConn* pConn = it->second;
        if (pConn && pConn != pFromConn && CHECK_CLIENT_TYPE_PC(pConn->GetClientType())) {
            pConn->SendPdu(pPdu);
        }
    }
}
...
void CImUser::BroadcastPduToMobile(CImPdu* pPdu, CMsgConn* pFromConn)
{
    ...
    {
        CMsgConn* pConn = it->second;
        if (pConn && pConn != pFromConn && CHECK_CLIENT_TYPE_MOBILE(pConn->GetClientType())) {
            pConn->SendPdu(pPdu);
        }
    }
}
...
void CImUser::BroadcastClientMsgData(CImPdu* pPdu, uint32_t msg_id, uint32_t to_id, uint32_t sequence_no, CMsgConn* pFromConn, uint32_t from_id)
{
    for (map<uint32_t, CMsgConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        CMsgConn* pConn = it->second;
        if (pConn && pConn != pFromConn)
		{
			pConn->AddToSendList(msg_id, from_id, to_id, sequence_no, (IM::BaseDefine::SessionType)SESSION_TYPE_SINGLE); // add a message to send list(to_id = 0 meas to the user itself)
			if (pConn->GetSendList() == 1 && !pConn->IsKickOff())
			{
				pConn->SendPdu(pPdu);
				//pConn->DelFromSendList(msg_id, from_id, to_id, sequence_no, (IM::BaseDefine::SessionType)SESSION_TYPE_SINGLE);
				pConn->SetSentStatus(false);
				log("############# SENDING PACKET TO CONNECTIONS(EXCEPT ITSELF) FOR USER: %d,SEQ: %d ###############", from_id, sequence_no);
			}
			else
			{
				log("ready to resend a single message");
				pConn->ResendMessageFromSendList();
			}
        }
    }
}
...
void CImUser::BroadcastData(void *buff, uint32_t len, CMsgConn* pFromConn)
{
    ...
    for (map<uint32_t, CMsgConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
        ...
        if (pConn != pFromConn) 
		{
            int nBytes = pConn->Send(buff, len);
			log("--------sending %d bytes to the %d 's connection:%d-------", nBytes, pConn->GetUserId(), it->first);
        }
    }
}


void CImUser::BroadcastData(void* buff, uint32_t len, uint32_t msg_id, uint32_t to_id, uint32_t sequence_no, CMsgConn* pFromConn, uint32_t from_id)
{
	if (!buff)
		return;
	...
	for (map<uint32_t, CMsgConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
	{
		...
		if (pConn != pFromConn)
		{
			...
			pConn->AddToSendList(msg_id, from_id, to_id, sequence_no,(IM::BaseDefine::SessionType)SESSION_TYPE_GROUP);
			if (pConn->GetSendList() == 1 && !pConn->IsKickOff()) //just a message to be sent
			{
				int nBytes = pConn->Send(buff, len);
				//pConn->DelFromSendList(msg_id, from_id, to_id, sequence_no, (IM::BaseDefine::SessionType)SESSION_TYPE_GROUP);
				pConn->SetSentStatus(false);
				log("--------sending %d bytes to the %d 's connection:%d, sequence no: %d-------", nBytes, pConn->GetUserId(), it->first, sequence_no);
			}
			else
			{
				log("ready to resend a group message.......");
				pConn->ResendMessageFromSendList();
			}
		}
	}
...
void CImUser::HandleKickUser(CMsgConn* pConn, uint32_t reason)
{
    ...
    if (it != m_conn_map.end()) {
        CMsgConn* pCurrConn = it->second;
        if(pCurrConn)
		{
            log("kick service user, user_id=%u, handle:%d", m_user_id, it->first);
            ...
            int nBytes= pCurrConn->SendPdu(&pdu);
			log("send bytes to client for kicking service: %d", nBytes);
            pCurrConn->SetKickOff();
        }
    }
}
    ...
    for (map<uint32_t, CMsgConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
    {
		...
		if (pMsgConn)
		{
			if ((((pMsgConn->GetClientType() ^ client_type) >> 4) == 0) && (pMsgConn != pFromConn))
			{
				...
				bKickUser = true;
				//break; //commented by george on Dec 3rd. maybe network problem leds abnormal disnnect  
			}
		}
    }
    return bKickUser;
...
void CImUserManager::BroadcastPdu(CImPdu* pdu, uint32_t client_type_flag, CMsgConn *pMsgConn)
{
    CImUser* pImUser = NULL;
    for (ImUserMap_t::iterator it = m_im_user_map.begin(); it != m_im_user_map.end(); it++)
    {
...
void CImUser::BroadcastPduByLoginTime(CImPdu* pPdu)
{
	uint64_t ulCurrentTime = get_tick_count();
	for (map<uint32_t, CMsgConn*>::iterator it = m_conn_map.begin(); it != m_conn_map.end(); it++)
	{
		CMsgConn* pConn = it->second;
		if (pConn && pConn->GetUserId() != 0 && ulCurrentTime - pConn->GetLoginTime() > 10000 )
		{
			pConn->SendPdu(pPdu);
			log("3.============================================User: %d, sending a packet to %d=============================", m_user_id, pConn->GetUserId());
		}
