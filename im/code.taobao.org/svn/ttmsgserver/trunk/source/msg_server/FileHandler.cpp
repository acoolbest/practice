...
//    log("HandleClientFileRequest, %u->%u, fileName: %s, trans_mode: %u, file size:%d, taskid: %s", from_id, to_id, file_name.c_str(), trans_mode, file_size, strTaskID.c_str());
//    
...
//
//			log("send message to file server:");
//
//        }
...
//        msg2.set_task_id(strTaskID);
//        msg2.set_trans_mode((IM::BaseDefine::FileType)trans_mode);
//		msg2.set_task_create_time(time(NULL));
//		msg2.set_valid_period(0);
//		msg2.set_file_id("");
//        CImPdu pdu;
//        pdu.SetPBMsg(&msg2);
...
//    string file_name = msg.file_name();
//    uint32_t file_size = msg.file_size();
//	uint32_t nCount = msg.ip_addr_list_size();
//	string file_id = msg.file_id();
//	log("HandleClientFileAddOfflineReq, %u->%u, task_id: %s, file_name: %s, size: %u, file_servers: %d, file_id:%s  ",
//		from_id, to_id, task_id.c_str(), file_name.c_str(), file_size, nCount, file_id.c_str());
//    
//	CDbAttachData attach_data(ATTACH_TYPE_HANDLE, pMsgConn->GetHandle(), 0);
//    CDBServConn* pDbConn = get_db_serv_conn();
//    if (pDbConn) {
...
//			pUser->BroadcastPdu(&pdu); //change to send notify to all online terminals
//        }
...
//    if (pFileConn)
//    {
//		log("ready to get file server ip list....................");
...
//            ip_addr->set_port(ip_addr_tmp.port());
//        }
//		log("Total file ip: %d", msg.ip_addr_list_size());
//    }
//    else
//    {
...
//
//
//void CFileHandler::HandleClientFileAddOfflineRes(CImPdu* pPdu)
//{
//	IM::File::IMFileAddOfflineRsp msg;
//	CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));
//	uint32_t req_user_id = msg.from_user_id();
//	string task_id = msg.task_id();
//	uint32_t nResult = msg.result_code();
//	CDbAttachData attach((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());
//
//	log("HandleFileAddOfflineRes, req_id=%u, task_id=%s, result_code:%d", req_user_id, task_id.c_str(), nResult);
//
//	CMsgConn* pConn = CImUserManager::GetInstance()->GetMsgConnByHandle(req_user_id,
//		attach.GetHandle());
//
...
//	pdu.SetPBMsg(&msg);
//	pdu.SetServiceId(SID_FILE);
//	pdu.SetCommandId(CID_FILE_ADD_OFFLINE_RES);
//	pdu.SetSeqNum(pPdu->GetSeqNum());
//
//	if (pConn)
//	{
//		//pConn->SendPdu(pPdu);
//		int nBytes = pConn->SendPdu(&pdu);
//		log("Add offline response: Handle: %d, %d bytes sent", attach.GetHandle(), nBytes);
//	}
//}