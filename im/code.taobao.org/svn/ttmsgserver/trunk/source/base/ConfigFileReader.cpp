...
CConfigFileReader::CConfigFileReader(const char* filename, bool bTruncate)
{
	m_config_map = new map<string, string>;
	_LoadFile(filename, bTruncate);
}
...
void CConfigFileReader::_LoadFile(const char* filename, bool bRemove)
{
    m_config_file.clear();
...
map<string, string>* CConfigFileReader::GetConfigItems(void)
{
	return m_config_map;
}