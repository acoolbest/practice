...
insert into IMUser (id,status,useruuid,username,name,keyword,nick,avatar,created,updated,buyproduct) values(1,1,'workflow','workflow','审批助手','workflow','审批助手','assistant_workflow',unix_timestamp(),unix_timestamp(),'IM,WORKFLOW');
# #initialize email
insert into IMUser (id,status,useruuid,username,name,keyword,nick,avatar,created,updated,buyproduct) values(2,1,'email','email','邮件助手','email','邮件助手','assistant_mail',unix_timestamp(),unix_timestamp(),'IM,EMAIL');
# #initialize bulltin
insert into IMUser (id,status,useruuid,username,name,keyword,nick,avatar,created,updated,buyproduct) values(3,1,'bulletin','bulletin','公告助手','bulletin','公告助手','assistant_bulletin',unix_timestamp(),unix_timestamp(),'IM,BULLETIN');
# new friend
insert into IMUser (id,status,useruuid,username,name,keyword,nick,avatar,created,updated,buyproduct) values(4,1,'friend','friend','新的好友','friend','新的好友','assistant_friend',unix_timestamp(),unix_timestamp(),'IM');
# new staff
insert into IMUser (id,status,useruuid,username,name,keyword,nick,avatar,created,updated,buyproduct) values(5,1,'staff','staff','成员申请','staff','成员申请','assistant_member',unix_timestamp(),unix_timestamp(),'IM');
#attendance
insert into IMUser (id,status,useruuid,username,name,keyword,nick,avatar,created,updated,buyproduct) values(6,1,'attendance','attendance','考勤助手','attendance','考勤助手','assistant_hr',unix_timestamp(),unix_timestamp(),'IM');
...
#
insert into IMPosition (uuid,companyid,sort,created,updated) values('00000000000000000000000000000000',0,1,unix_timestamp(),unix_timestamp());
# 
##user position
...
#insert into IMCompany (domainid,domainname,status,created,updated,syncserver) select domain_id,domain_name,status,unix_timestamp(),unix_timestamp(),server_name from coa.domain;
...
#declare currproduct varchar(40);
#declare  curruserid varchar(40);
#declare orderproduct varchar(256);
#set curruserid='';
#DECLARE rs_cursor CURSOR FOR SELECT a.user_id,b.product_name from coa.user_product a, coa.product b where a.domain_product_id = b.product_id order by a.user_id;
#open rs_cursor;
#cursor_loop:loop
#   FETCH rs_cursor into userid, currproduct;
#   if curruserid = userid
#   		concat(orderproduct,',',currproduct);
#   else
#   		if curruserid <> ''
#   			update IMUserA set buyproduct=orderproduct,updated=unix_timestamp() where useruuid=curruserid;
#   		end if;
#   		curruserid = userid;
...
#<img alt="\" src="http://www.2cto.com/uploadfile/Collfiles/20140821/201408211031296.jpg" style="width: 462px; height: 365px;">
#实例一
# 
...
#select s;
#end $      </pre>
#<img alt="\" src="http://www.2cto.com/uploadfile/Collfiles/20140821/201408211031297.jpg" style="width: 492px; height: 373px;"><br>
#<br>
#存储过程和函数的区别： 名称不同 ：存储过程：procedure 函数function 存储过程没有返回值。<br>
#实例二<br>
#一个简单存储过程游标实例<br>
#DELIMITER $$<br>
