...
typedef list<transfer_task_t*> LIST_TRANSFER_TASK;
typedef LIST_TRANSFER_TASK::iterator IT_LIST_TRANSFER_TASK;

//typedef map<std::string, transfer_task_t*> TaskMap_t; // on client connect
typedef map<std::string, LIST_TRANSFER_TASK> TaskMap_t; // on client connect
...
void init_file_conn(std::list<IM::BaseDefine::IpAddr>&, uint32_t timeout, uint32_t nValidPeriod);
