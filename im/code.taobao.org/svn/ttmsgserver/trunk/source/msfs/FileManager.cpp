...
//							char *url, char *ext)
int FileManager::uploadFile(const char *type, const void *content, u32 size, char *url, const char *pszOriginalPath, char *ext)
{
	//check file size
	if (size > (MAX_FILE_SIZE_PER_FILE*2)) {
		log("File size[%d] should less than [%d]", size, 
			(MAX_FILE_SIZE_PER_FILE*2));
		return -1;
	}
	...
	string path = "";
	if (pszOriginalPath == NULL)
	{
		path = createFileRelatePath();
		log("CREATE A PATH: %s", path.c_str());
	}
	else
	{
		path = pszOriginalPath;
		int nPosition = path.find("g0");
		if (nPosition != string::npos)
		{
			path = path.substr(nPosition + 2);
		}

		log("EXISTING PATH: %s", path.c_str());
	}
