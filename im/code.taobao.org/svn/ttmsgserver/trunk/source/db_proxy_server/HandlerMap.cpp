...
#include "business/CompanyAction.h"
#include "business/FileAction.h"
#include "business/PositionAction.h"
...
void CHandlerMap::Init()
{
	...
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_USER_UPDATE_NAGURE_REQUEST), DB_PROXY::UpdateUserNagure));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_USER_UPDATE_INFO_REQUEST), DB_PROXY::UpdateUser));
    m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_DEPARTMENT_REQUEST), DB_PROXY::getChgedDepart));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_COMPANY_REQUEST), DB_PROXY::getCompanyInfo));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_CHANGE_AVATAR_REQUEST), DB_PROXY::updateUserAvatar));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_POSITION_REQUEST), DB_PROXY::getChangedPosition));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_ALL_USER_LITE_REQUEST), DB_PROXY::getChangedUserLite));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_USER_INFO_LITE_REQUEST), DB_PROXY::getUserInfoLite));
	//added by george on July 5th,2016
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_STAFF_INFO_REQUEST), DB_PROXY::GetStaffInfo));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_ALL_STAFF_REQUEST), DB_PROXY::GetChangedStaff));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_ALL_STAFF_LITE_REQUEST), DB_PROXY::GetChangedStaffLite));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_STAFF_UPDATE_INFO_REQUEST), DB_PROXY::UpdateStaff));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_CHANGE_VIP_REQUEST), DB_PROXY::ChangeVip));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_ASST_INFO_REQUEST), DB_PROXY::GetAssistantInfo));
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_LIST_QUERY_USER_INFO_REQUEST), DB_PROXY::QueryUserInfo));
	...
	m_handler_map.insert(make_pair(uint32_t(CID_MSG_RESEND_MSG_REQ), DB_PROXY::getResendMessageById)); //handler for resending a message is same as getmsgbyid.added by george on Dec 30th,2015
	...
	m_handler_map.insert(make_pair(uint32_t(CID_BUDDY_SHIELD_USER_REQUEST), DB_PROXY::setUserPush));
    m_handler_map.insert(make_pair(uint32_t(CID_OTHER_GET_SHIELD_REQ), DB_PROXY::getGroupPush));
	m_handler_map.insert(make_pair(uint32_t(CID_OTHER_GET_SHIELD_USER_REQ), DB_PROXY::getUserPush));
	...
	m_handler_map.insert(make_pair(uint32_t(CID_GROUP_CHANGE_GRPNAME_REQUEST), DB_PROXY::modifyGroupName));
    m_handler_map.insert(make_pair(uint32_t(CID_GROUP_TRANSFER_LEADER_REQUEST), DB_PROXY::modifyGroupLeader));
