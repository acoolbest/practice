...
namespace DB_PROXY {
    ...
    //            *pContact = *it;
				...
				pContact->set_session_name(it->session_name());
				pContact->set_avatar_url(it->avatar_url());
				pContact->set_last_at_msgid(it->last_at_msgid());
				//log("session id:%lu, update time:%lu,latest message id:%lu,session status:%lu", it->session_id(), it->updated_time(), it->latest_msg_id(), it->session_status());
            }
