 ...
 */
bool CGroupMessageModel::sendMessage(uint32_t nFromId, uint32_t nGroupId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime,uint32_t nMsgId, const string& strMsgContent, uint32_t unFlag)
{
    bool bRet = false;
    if(nFromId == 0 || CGroupModel::getInstance()->isInGroup(nFromId, nGroupId))
    {
        ...
        if (pDBConn)
        {
            ...
            string strSql = "insert into " + strTableName + " (`groupId`, `userId`, `msgId`, `content`, `type`, `status`, `updated`, `created`, flag) "\
            "values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if (pStmt->Init(pDBConn->GetMysql(), strSql))
            {
                ...
                if (bRet)
                {
                    ...
                    //clearMessageCount(nFromId, nGroupId); george commented the line on Oct 21st,2015
                } else {
 ...
 *  @return 成功返回true，失败返回false
 */
bool CGroupMessageModel::sendAudioMessage(uint32_t nFromId, uint32_t nGroupId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, const char* pMsgContent, uint32_t nMsgLen, uint32_t unFlag)
{
	if (nMsgLen <= 4) {
		return false;
	}
	...
	if (nAudioId != -1) {
        ...
        bRet = sendMessage(nFromId, nGroupId, nMsgType, nCreateTime, nMsgId, strMsg, unFlag);
	} else {
		bRet = false;
	}
...
bool CGroupMessageModel::sendFileMessage(uint32_t nFromId, uint32_t nGroupId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, const char* pMsgContent, uint32_t nMsgLen, string& strFileInfo, uint32_t unFlag)
{
	if (nMsgLen <= 4) {
		return false;
	}
	...
	if (!nResult)
	{
		...
		{  
			string strCurrentTask = root["task_id"].asString();  
			...
			{
				pFileModel->AddOfflineFile(nFromId, nGroupId, strFileId, strFileName, unFileSize, IM::BaseDefine::SESSION_TYPE_GROUP, unCreatedTime, unValidTime);
				strFileInfo = pMsgContent;
				bResult = sendMessage(nFromId, nGroupId, nMsgType, nCreateTime, nMsgId, strFileInfo, unFlag);
			}
			else if (pFileModel && strCurrentTask != "")
			{
				bResult = pFileModel->getOfflineFileDetails(strCurrentTask, strCurrentFile, strFileInfo);
				if (bResult)
				{
					bResult = sendMessage(nFromId, nGroupId, nMsgType, nCreateTime, nMsgId, strFileInfo, unFlag);
				}
    ...
    if (pCacheConn)
    {
        ...
        if(bRet)
        {
            ...
            if(!strReply.empty())
            {
				...
				{
					log("INCREASE, KEY: %s, FIRST: %s, SECOND: %s", strUserKey.c_str(), it->first.c_str(), it->second.c_str());
				}
...
void CGroupMessageModel::getMessage(uint32_t nUserId, uint32_t nGroupId, uint32_t nMsgId, uint32_t nMsgCnt, list<IM::BaseDefine::MsgInfo>& lsMsg)
{
    ...
    if (pDBConn)
    {
        ...
        if(nMsgId == 0)
        {
            strSql = "select * from " + strTableName + " where groupId = " + int2string(nGroupId) + " and status = 0 and created>="+ int2string(nUpdated) + " order by id desc, created desc limit " + int2string(nMsgCnt);
        }else {
            strSql = "select * from " + strTableName + " where groupId = " + int2string(nGroupId) + " and msgId<=" + int2string(nMsgId) + " and status = 0 and created>="+ int2string(nUpdated) + " order by id desc, created desc limit " + int2string(nMsgCnt);
        }
        ...
        if (pResultSet)
        {
            ...
            while(pResultSet->Next())
            {
				...
				msg.set_msg_flag(pResultSet->GetInt("flag"));
...
uint32_t CGroupMessageModel::getMsgId(uint32_t nGroupId)
{
    ...
    if(pCacheConn)
    {
		...
		if (!pCacheConn->isExists(strKey)) //key is not found,we should get maximum msgid from db
		{
			CDBManager* pDBManager = CDBManager::getInstance();
			CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
			if (pDBConn)
			{
				string strSql = "select max(msgid) as maxmsgid from IMGroupMessage_" + int2string(nGroupId % 8) + " where groupid=" + int2string(nGroupId);
				CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
				log("KEY IS NOT FOUND IN THE CACHE,GET MSGID FROM DB: %s", strSql.c_str());
				if (pResultSet)
				{
					while (pResultSet->Next())
					{
						if (pResultSet->GetString("maxmsgid") != NULL)
						{
							...
							log("MAX MSGID FOR GROUP %d is %d", nGroupId, nMsgId);
						}
						else
						{
							log("ALSO,NO FOUND IN DB,SET TO 0");
							nMsgId = 0;
						}
					}
					delete pResultSet;
					...
					//pCacheConn->incrBy(strKey, nMsgId); //set msg id
				}
				pDBManager->RelDBConn(pDBConn);
			}
		}
...
//                log("hget %s : count failed !", strGroupKey.c_str());
			...
			log("GROUP %d COUNT: %d, USER %d COUNT: %d, DEFICIT: %d, TOTAL COUNT: %d", nGroupId, nGroupCnt, nUserId, nUserCnt, nCount, nTotalCnt);
        }
        pCacheManager->RelCacheConn(pCacheConn);
    }
    else
...
void CGroupMessageModel::getMsgByMsgId(uint32_t nUserId, uint32_t nGroupId, const list<uint32_t> &lsMsgId, list<IM::BaseDefine::MsgInfo> &lsMsg, bool bAudioData)
{
    if(!lsMsgId.empty())
    {
