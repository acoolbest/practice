...
#define FILE_STATUS_READY		0
#define FILE_STATUS_RECEIVED	1
...
public:
    ...
    bool sendMessage(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime,
                     uint32_t nMsgId, string& strMsgContent, int32_t nFlag = 0);
	...
	bool sendFileMessage(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime,
                          uint32_t nMsgId, const char* pMsgContent, uint32_t nMsgLen, string& strFileInfo);
	uint32_t sendFileStatusMessage(uint32_t nRelateId, uint32_t unUserId, const char* pMsgContent, uint32_t nMsgLen);
    ...
    void getMsgByMsgId(uint32_t nUserId, uint32_t nPeerId, const list<uint32_t>& lsMsgId, list<IM::BaseDefine::MsgInfo>& lsMsg, bool bAudioData = true);
