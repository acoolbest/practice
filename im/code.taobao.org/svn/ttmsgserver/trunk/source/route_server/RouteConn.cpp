...
void CRouteConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
		...
		case CID_BUDDY_LIST_USER_MOMENT_NOTIFY:
			log("COMMAND ID: %d", pPdu->GetCommandId());
            _BroadcastMsg(pPdu, this);
            break;
        
	default:
		log("CRouteConn::HandlePdu, wrong cmd id: %d ,CID_BUDDY_LIST_USERS_STATUS_REQUEST=%d", pPdu->GetCommandId(),CID_BUDDY_LIST_USERS_STATUS_REQUEST);
		break;
	}
}
...
void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu)
{
   ...
   // if (CHECK_CLIENT_TYPE_PC(client_type)) //george jiang commented the judgement on July 8th, 2015
    {
        ...
        if (pUser)
        {
			...
			if (USER_STATUS_OFFLINE == user_status && !pUser->IsMsgConnNULL())
            {
                return;
            }
            else
...
void CRouteConn::_HandleUsersStatusRequest(CImPdu* pPdu)
{
    ...
    for(uint32_t i = 0; i < query_count; i++)
	{
		...
		//log("Userid: %d, Status: %d", msg2.user_stat_list(i).user_id(), msg2.user_stat_list(i).status());
	}
...
void CRouteConn::_BroadcastMsg(CImPdu* pPdu, CRouteConn* pFromConn)
{
	...
	for (it = g_route_conn_map.begin(); it != g_route_conn_map.end(); it++)
	{
		...
		if (pRouteConn && pRouteConn != pFromConn)
		{
			int nBytes = pRouteConn->SendPdu(pPdu);
			log("Sent %d bytes to the other side", nBytes);
		}
	}
}
...
void CRouteConn::_HandleOnlineUsersRequest(CImPdu* pPdu)
{
	IM::Buddy::IMOnlineUsersReq msg;
	CHECK_PB_PARSE_MSG(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()));

	uint32_t request_id = msg.user_id();
	log("HandleOnlineUsersReq, req_id=%u", request_id);

	IM::Buddy::IMOnlineUsersRsp msg2;
	msg2.set_user_id(request_id);
	msg2.set_attach_data(msg.attach_data());
	CUserInfo* pUser = NULL;
	for (UserInfoMap_t::iterator it = g_user_map.begin(); it != g_user_map.end(); it++)
	{
		pUser = it->second;
		//::IM::BaseDefine::UserStatType)
		if (pUser && pUser->GetStatus() == USER_STATUS_ONLINE)
		{
			msg2.add_user_id_list(it->first);
			log("Userid: %d, Status: %d", it->first, pUser->GetStatus());
		}		
	}
	...
	pdu.SetPBMsg(&msg2);
	pdu.SetServiceId(SID_BUDDY_LIST);
	pdu.SetCommandId(CID_BUDDY_LIST_ONLINE_USER_RESPONSE);
	pdu.SetSeqNum(pPdu->GetSeqNum());
	SendPdu(&pdu);
}