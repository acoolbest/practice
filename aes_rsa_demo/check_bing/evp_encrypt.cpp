...
static uint32_t save_aes_key_iv(string save_path, const uint8_t *key, const uint8_t *iv, bool encrypt = true)
{
	...
	if(encrypt)
	{
		...
		{
			for(int key_i=0;key_i<cipher_key.length();key_i++)
			{
				printf("%0d ", cipher_key[key_i]);
			}
			std::cout << std::endl;
...
//	memcpy(g_local_iv, "0000000000000000", BLOCK_SIZE);
	memcpy(g_local_key, "ikjhdbsncgtrhdjg", KEY_SIZE);
	memcpy(g_local_iv, "hdgsftahtgfjdhgr", BLOCK_SIZE);
	#endif
